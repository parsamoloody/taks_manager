...
export function LabelManagement({ labels }: { labels: LabelDto[] }) {
  const createMutation = useMutation<{ ok: boolean; message?: string }>();
  const editMutation = useMutation<{ ok: boolean; message?: string }>();
  const deleteMutation = useMutation<{ ok: boolean; message?: string }>();
  ...
  return (
    <section className="border-t border-white/10 pt-6">
      ...
      <p className="mt-1 text-xs leading-5 text-slate-400">
        Create reusable labels for organizing tasks on this board.
      </p>

      <MutationForm
        mutation={createMutation}
        className="mt-4 grid grid-cols-[1fr_3rem] gap-2"
      >
        ...
        <FormInput
          ...
          className="h-[42px] cursor-pointer px-1.5 py-1"
        />
        ...
        <Button
          type="submit"
          variant="secondary"
          isLoading={createMutation.state !== "idle"}
          ...
          >
            {editingId === label.id ? (
              <MutationForm
                ...
                className="grid grid-cols-[1fr_3rem] gap-2"
              >
                ...
                <div className="col-span-2 flex gap-2">
                  <Button
                    type="submit"
                    variant="primary"
                    isLoading={editMutation.state !== "idle"}
                    className="flex-1"
                  >
                    Save
                  </Button>
              ...
              <div className="flex items-center gap-2">
                ...
                <MutationForm mutation={deleteMutation}>
      ...
      {editMutation.data && !editMutation.data.ok ? (
        <p role="alert" className="mt-2 text-xs text-rose-400">
          {editMutation.data.message}
        </p>
      ) : null}
