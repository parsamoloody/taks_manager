...
export interface BoardDetailList extends List {
  tasks: Task[];
}
...
export function getBoard(token: string, workspaceId: string, boardId: string) {
  return requestJson<Board>(`board/${workspaceId}/${boardId}`, {
    method: "GET",
    token,
  });
}
...
export function createBoard(
  ...
  return requestJson<Board>(`board/${workspaceId}`, {
    method: "POST",
    json: payload,
    token,
  });
...
export function updateBoard(
  ...
  return requestJson<Board>(`board/${boardId}`, {
    method: "PUT",
    json: payload,
    token,
  });
